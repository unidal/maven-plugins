...
public class DalJdbcSupport extends ComponentTestCase {
   ...
   private File generateSources(MyHelper helper) throws Exception {
      ...
      DalJdbcMojo mojo = builderOf(DalJdbcMojo.class, helper) //
            .component("m_generator", Generator.class, "dal-jdbc").build();
      final AtomicReference<File> manifest = new AtomicReference<File>();

      Scanners.forDir().scan(new File(baseDir, "jdbc"), new FileMatcher() {
         @Override
   ...
   private static class MyHelper {
      ...
      public MyHelper(String scenario) {
         m_scenario = scenario;
         m_baseDir = new File("src/test/dal-jdbc/" + scenario);
