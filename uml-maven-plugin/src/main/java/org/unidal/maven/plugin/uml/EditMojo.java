...
public class EditMojo extends AbstractMojo {
   ...
   public void execute() throws MojoExecutionException, MojoFailureException {
      ...
      try {
         ...
         throw new MojoExecutionException("Error when starting up server at " + warRoot + "!", e);
      }
   }
   ...
   private String getWarRoot() {
      ...
      if (protocol.equals("jar")) {
         ...
         if (path.startsWith("file:")) {
            ...
            try {
               Files.forZip().copyDir(new ZipInputStream(new FileInputStream(jarPath)), target, new Policy() {
                  @Override
               ...
               throw new RuntimeException(String.format("Error when extracting jar(%s) to %s!", jarPath, target));
            }
         }
      }
