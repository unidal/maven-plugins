...
<xsl:template match="model">
   ...
   <xsl:value-of select="$empty"/>public class DefaultNativeParser2 implements IVisitor {<xsl:value-of select="$empty-line"/>
...
<xsl:template name="method-commons">
   ...
   public DefaultNativeParser2(InputStream in) {
   ...
   public static <xsl:value-of select="$root/@entity-class"/> parse(InputStream in) {
      DefaultNativeParser2 parser = new DefaultNativeParser2(in);
