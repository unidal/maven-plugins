...
<xsl:template match="model">
   ...
   <xsl:value-of select="$empty"/>public abstract class BaseFilter implements IFilter {<xsl:value-of select="$empty-line"/>
...
<xsl:template name="method-visit">
   <xsl:if test="entity/any">
      ...
      <xsl:value-of select="$empty"/>   public void <xsl:value-of select="entity/any/@visit-method"/>(Any any, IVisitor chain) {<xsl:value-of select="$empty-line"/>
      <xsl:value-of select="$empty"/>   }<xsl:value-of select="$empty-line"/>
   </xsl:if>
   <xsl:for-each select="entity">
      ...
      <xsl:value-of select="$empty"/>   @Override<xsl:value-of select="$empty-line"/>
      <xsl:value-of select="$empty"/>   public void <xsl:value-of select="@visit-method"/>(<xsl:value-of select="@entity-class"/><xsl:value-of select="$space"/><xsl:value-of select="@param-name"/>, IVisitor chain) {<xsl:value-of select="$empty-line"/>
      <xsl:value-of select="$empty"/>      chain.<xsl:value-of select="@visit-method"/>(<xsl:value-of select="@param-name"/>);<xsl:value-of select="$empty-line"/>
      <xsl:value-of select="$empty"/>   }<xsl:value-of select="$empty-line"/>
