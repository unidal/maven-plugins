...
public class <xsl:value-of select="@jsp-viewer-class"/> extends BaseJspViewer<xsl:call-template name="generic-type"><xsl:with-param name="type" select="$type"/></xsl:call-template> {
   ...
   protected String getJspFilePath(<xsl:value-of select="@context-class"/> ctx, <xsl:value-of select="@model-class"/> model) {
      ...
      switch (action) {
      case LIST:
         return <xsl:value-of select="@jsp-file-class"/>.LIST.getPath();
<xsl:if test="$policy-add='true'">
   <xsl:value-of select="$empty"/>      case ADD:<xsl:value-of select="$empty-line"/>
</xsl:if>
<xsl:if test="$policy-edit='true'">
   <xsl:value-of select="$empty"/>      case EDIT:<xsl:value-of select="$empty-line"/>
</xsl:if>
<xsl:value-of select="$empty"/>      case VIEW:
         return <xsl:value-of select="@jsp-file-class"/>.DETAIL.getPath();
      }
