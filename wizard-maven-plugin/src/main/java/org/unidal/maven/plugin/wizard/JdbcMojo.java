...
public class JdbcMojo extends AbstractMojo {
    ...
    * @readonly
    */
   private Obfuscater m_obfuscater;
    ...
    * 
    * @parameter expression="${manifest}" default-value= "${basedir}/src/main/resources/META-INF/wizard/jdbc/manifest.xml"
    * @required
    */
   ...
   private Pair<Wizard, Jdbc> buildWizard(File wizardFile) throws IOException, SAXException {
      Wizard wizard;
   ...
   private void generateModel(Group group) throws SQLException, IOException {
      ...
      Document codegenDoc = new Document(entities);
   ...
   void modifyPomFile(File pomFile, Wizard wizard, Jdbc jdbc) throws JDOMException, IOException {
      Document doc = new SAXBuilder().build(pomFile);
      boolean modified = new PomBuilder(wizard).build(doc, jdbc);
   ...
   private class ConnectionMaker {
      public Connection makeConnection(Datasource ds) {
   ...
   private class PomBuilder {
      ...
      public boolean build(Document doc, Jdbc jdbc) {
         Element root = doc.getRootElement();
         PomDelegate b = new PomDelegate();
         Element dependencies = b.findOrCreateChild(root, "dependencies");

         if (!b.checkDependency(dependencies, "org.unidal.framework", "dal-jdbc", "6.0.0", null)) {
            b.checkDependency(dependencies, "org.unidal.cat", "cat-all", "2.0.0", null);
            b.checkDependency(dependencies, "mysql", "mysql-connector-java", "5.1.49", "runtime");
         }

         if (jdbc != null) {
            Element build = b.findOrCreateChild(root, "build", null, "dependencies");
            Element plugins = b.findOrCreateChild(build, "plugins");
            Element codegenPlugin = b.checkPlugin(plugins, "org.unidal.maven.plugins", "codegen-maven-plugin", "5.0.0");
            Element codegenGenerate = b.checkPluginExecution(codegenPlugin, "dal-jdbc", "generate-sources",
                  "generate dal jdbc files");
            ...
            Element plexusPlugin = b.checkPlugin(plugins, "org.unidal.maven.plugins", "plexus-maven-plugin", "5.0.0");
            ...
            b.findOrCreateChild(codegenPlexusConfiguration, "className")
                  .setText(m_wizard.getPackage() + ".build.ComponentsConfigurator");
         }
   ...
   private class WizardBuilder extends BaseVisitor {
      ...
      public void visitGroup(final Group group) {
         ...
         PropertyProviders.fromConsole().forString("table", "Select table name below, or 'end':", availableTableNames,
      ...
      public void visitJdbc(Jdbc jdbc) {
         ...
         String groupName = PropertyProviders.fromConsole().forString("group",
               "Select group name below or input a new one:", groupNames, null, null);
      ...
      public void visitWizard(Wizard wizard) {
         ...
         String name = PropertyProviders.fromConsole().forString("datasource",
               "Select datasource name below or input a new one:", names, null, null);
         ...
         if (jdbc == null) {
            ...
            ds.setDriver(
                  PropertyProviders.fromConsole().forString("driver", "JDBC driver:", "com.mysql.jdbc.Driver", null));
            ds.setUrl(PropertyProviders.fromConsole().forString("url", "JDBC URL:",
                  "jdbc:mysql://localhost:3306/" + name, null));
            ds.setUser(PropertyProviders.fromConsole().forString("user", "User:", null, null));

            String password = PropertyProviders.fromConsole().forString("password",
                  "Password:(use '<none>' if no password)", null, null);
            ...
            ds.setProperties(PropertyProviders.fromConsole().forString("connectionProperties", "Connection properties:",
                  "useUnicode=true&characterEncoding=UTF-8&autoReconnect=true&useSSL=false&verifyServerCertificate=false",
                  null));
         }

         m_jdbc = jdbc;
         m_conn = new ConnectionMaker().makeConnection(jdbc.getDatasource());
