...
public class ModelMojo extends AbstractMojo {
   ...
   class WizardBuilder extends BaseVisitor {
      ...
      public void visitWizard(final Wizard wizard) {
         ...
         String name = PropertyProviders.fromConsole().forString("model", "Select model name below or input a sample xml file:",
               names, null, new IValidator<String>() {
                  @Override
         ...
         if (model == null) {
            try {
               ...
               String prefix = PropertyProviders.fromConsole().forString("prefix", "Prefix name of target files:", rootName,
               ...
               String packageName = PropertyProviders.fromConsole().forString("package", "Package name of generated model:",
                     wizard.getPackage() + "." + prefix, null);
            ...
            if (!file.exists()) {
               String sampleModel = PropertyProviders.fromConsole().forString("model.sample",
                     String.format("Sample model(%s) does not exist, please input a new one:", model.getSampleModel()), null, null);
